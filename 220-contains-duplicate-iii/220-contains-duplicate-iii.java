class Solution {    
    public boolean containsNearbyAlmostDuplicate(int[] nums, int k, int t) {
        ...
        for (int i = 0; i < nums.length; i++) {
            ...
            if (buckets.containsKey(bucket - 1) && remappedNum - buckets.get(bucket - 1) <= t) return true;
            if (buckets.containsKey(bucket + 1) && buckets.get(bucket + 1) - remappedNum <= t) return true;
